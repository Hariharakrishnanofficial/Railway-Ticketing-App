/**
 * AdminLayout.jsx
 * Sidebar + topbar layout exclusively for Admin role.
 * Only visible to admin@admin.com (or Role === 'Admin').
 */
...
function Breadcrumb() {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
      <span style={{ color: 'var(--text-faint)', fontWeight: 500 }}>Admin</span>
      {segments.length === 0
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-base)' }}>
      ...
      <aside style={{
        ...
        <div style={{ padding: '20px 16px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 38, height: 38, borderRadius: 11, flexShrink: 0, background: 'linear-gradient(135deg,#ef4444,#dc2626)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 14px rgba(239,68,68,0.35)' }}>
          ...
          {!collapsed && (
            <div style={{ overflow: 'hidden', whiteSpace: 'nowrap' }}>
              ...
              <div style={{ fontSize: 10, color: '#ef4444', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' }}>Admin Panel</div>
            </div>
          )}
        </div>
        ...
        {/* Admin badge */}
        {!collapsed && (
          <div style={{ margin: '10px 12px 0', padding: '6px 10px', borderRadius: 8, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', display: 'flex', alignItems: 'center', gap: 7 }}>
            <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#ef4444', display: 'inline-block', flexShrink: 0 }} />
            <span style={{ fontSize: 11, fontWeight: 700, color: '#ef4444', fontFamily: FONT }}>ADMIN ACCESS</span>
          </div>
        )}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', marginLeft: sidebarW, transition: 'margin-left 0.25s cubic-bezier(.4,0,.2,1)', minWidth: 0 }}>
        ...
        <header style={{ height: 64, background: 'var(--bg-surface)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px', position: 'sticky', top: 0, zIndex: 100 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            {/* Admin indicator */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '4px 10px', borderRadius: 8, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)' }}>
              <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#ef4444', display: 'inline-block', animation: 'pulse 2s ease-in-out infinite' }} />
              <span style={{ fontSize: 12, color: '#ef4444', fontWeight: 700 }}>Admin</span>
            </div>
            <div style={{ width: 1, height: 22, background: 'var(--border)' }} />
            {/* User info */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 34, height: 34, borderRadius: 10, flexShrink: 0, background: 'linear-gradient(135deg, #ef4444, #dc2626)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, color: '#fff' }}>
                {avatarLetter}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.3 }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', whiteSpace: 'nowrap' }}>{user?.Full_Name || 'Admin'}</span>
                <span style={{ fontSize: 11, color: '#ef4444', whiteSpace: 'nowrap', fontWeight: 600 }}>Administrator</span>
              </div>
