function SignInModal({ onClose, onLogin }) {
  ...
  const inp = {
    ...
    color: '#e2e8f0', fontSize: 14, fontFamily: 'var(--font-body)',
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: '#0d1117', border: '1px solid #1a1f2e', borderRadius: 20, padding: 32, width: '100%', maxWidth: 400, boxShadow: '0 24px 80px rgba(0,0,0,0.6)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 22 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: '#4a5568', marginTop: 3 }}>RailAdmin Ticketing System</div>
          </div>
        ...
        <div style={{ display: 'flex', background: '#090c12', border: '1px solid #1a1f2e', borderRadius: 10, padding: 3, marginBottom: 22, gap: 3 }}>
          {['login', 'register'].map(t => (
            <button key={t} type="button" onClick={() => { setTab(t); setErrors({}); }}
              style={{ flex: 1, padding: '8px', borderRadius: 8, border: 'none', cursor: 'pointer', fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 600, transition: 'all 0.15s',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
          {tab === 'register' && (
            <div>
              ...
              <input name="Full_Name" value={form.Full_Name} onChange={handleChange} placeholder="Your full name"
                style={{ ...inp, borderColor: errors.Full_Name ? '#ef4444' : '#1a1f2e' }}
          ...
          <div>
            ...
            <input name="Email" value={form.Email} onChange={handleChange} type="email" placeholder="you@example.com"
              style={{ ...inp, borderColor: errors.Email ? '#ef4444' : '#1a1f2e' }}
              onFocus={e => { e.target.style.borderColor = '#3b82f6'; }}
          ...
          <div>
            ...
            <div style={{ position: 'relative' }}>
              <input name="Password" value={form.Password} onChange={handleChange}
                ...
                style={{ ...inp, paddingRight: 42, borderColor: errors.Password ? '#ef4444' : '#1a1f2e' }}
                onFocus={e => { e.target.style.borderColor = '#3b82f6'; }}
        ...
        <button type="button" onClick={handleSubmit} disabled={loading}
          style={{ width: '100%', marginTop: 20, padding: '12px', borderRadius: 12, border: 'none', fontFamily: 'var(--font-body)', fontSize: 14, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
            background: loading ? '#1a1f2e' : 'linear-gradient(135deg,#3b82f6,#8b5cf6)',
